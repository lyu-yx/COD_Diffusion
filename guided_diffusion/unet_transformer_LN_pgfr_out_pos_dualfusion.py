...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, in_pixels, linear_dim, num_features):
        ...
        self.LN_mlp = nn.LayerNorm([num_features, n_head * linear_dim * 2])
        #Scaled dot product attention
        self.attention = ScaledDotProductAttention(temperature=in_pixels ** 0.5)
        self.mhsa = ScaledDotProductAttention(temperature=in_pixels ** 0.5)
    ...
    def forward(self, v, k, q, mask=None, emb=None):
        ...
        n_attn = self.attention(v, k, q, mask=mask)
        e_attn = self.attention(k, k, q, mask=mask)
        ...
        n_attn = n_attn.transpose(1, 2).contiguous().view(b, c, n_head * linear_dim)
        e_attn = e_attn.transpose(1, 2).contiguous().view(b, c, n_head * linear_dim)
...
class PriorGuidedFeatureRefinement(nn.Module):
    ...
    def forward(self, edge_bb_feature):
        ...
        feature_out = feature_out + edge_bb_feature
